...
import MoonwalkerProfileSdk from "./profile-sdk";
...
describe("MoonwalkerProfileSdk", () => {
  describe("constructor", () => {
    ...
    it("uses custom provider instance if specified", () => {
      ...
      const sdk = new MoonwalkerProfileSdk({ provider: customProvider });
      expect(sdk.provider).toBe(customProvider);
    });
    it("uses specific chainId if chainId is provided", () => {
      const sdk = new MoonwalkerProfileSdk({ chainId: TESTNET_CHAIN_ID });
    ...
    describe("getProfile", () => {
      ...
      it("sets cookies", async () => {
        ...
        expect(Cookies.set).toBeCalledWith(
          ...
          {
            username: "Cheems",
            avatar: `https://moonwalker.network/images/nfts/${sleepyNft.images.sm}`,
          },
          { domain: "moonwalker.network", secure: true, expires: 30 }
        );
