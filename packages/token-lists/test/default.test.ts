...
import currentMoonwalkerswapDefaultList from "../lists/moonwalkerswap-default.json";
import { buildList, VersionBump } from "../src/buildList";

const currentLists = {
  "moonwalkerswap-default": currentMoonwalkerswapDefaultList,
};
...
expect.extend({
  ...
  toBeValidLogo(token) {
    ...
    const hasTWLogo =
      token.logoURI === `https://moonwalker.network/coins//${token.address}.png`;
    let hasLocalLogo = false;
    const refersToLocalLogo = token.logoURI === `https://moonwalker.network/coins/${token.address}.png`;
...
describe.each([["moonwalkerswap-default"]])(
  "buildList %s",
  (listName) => {
